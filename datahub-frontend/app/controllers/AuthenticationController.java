...
public class AuthenticationController extends Controller {
  ...
  public Result authenticate(Http.Request request) {
    ...
    // 3. If either JAAS auth or Native auth is enabled, fallback to it
  ...
  private Optional<Result> redirectToIdentityProvider(Http.RequestHeader request, String redirectPath) {
    ...
    try {
      ...
      _logger.error(
          ...
          e);
      return Optional.of(Results.redirect(String.format("/login?error_msg=%s", URLEncoder.encode(
  ...
  private Result createSession(String userUrnString, String accessToken) {
    return Results.ok()
        ...
        .withCookies(
            createActorCookie(userUrnString, _cookieConfigs.getTtlInHours(), _cookieConfigs.getAuthCookieSameSite(),
  ...
  private void tryProvision(CorpuserUrn urn) {
    ...
    CorpUserStatus status = new CorpUserStatus().setStatus(Constants.CORP_USER_STATUS_ACTIVE)
    ...
    entityClient.ingestProposal(proposal, systemAuthentication, false);
  }
}